class Admin::AttachmentsController < Admin::ApplicationController
  ...
  def new
    ...
    @attachment = params[:type].constantize.new(params[params[:type].underscore.to_sym])
    @attachment.save(false)

    respond_to do |format|
      format.html { render :partial => 'li', :object => @attachment}
      format.xml  { render :xml => @attachment }
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @attachment.save
        ...
        format.html { redirect_to([:admin,@attachment]) }
